...
//在安卓的包相爱sufrace的native包
ANativeWindow *nativeWindow;
//再opencv的头文件中，分类器的加载类
CascadeClassifier *faceClassifier;

extern "C"
JNIEXPORT void JNICALL
Java_com_face_water_MainActivity_loadModel(JNIEnv *env, jobject instance, jstring detectModel_) {
...
Java_com_face_water_MainActivity_process(JNIEnv *env, jobject instance, jobject bitmap) {
    int ret = 1;
    Mat src;
...
JNIEXPORT void JNICALL
Java_com_face_water_MainActivity_setSurface(JNIEnv *env, jobject instance, jobject surface, jint w,
                                            jint h) {
    if(surface && w && h) {
        if (nativeWindow) {
...
JNIEXPORT void JNICALL
Java_com_face_water_MainActivity_destroy(JNIEnv *env, jobject instance) {
    if(faceClassifier) {
